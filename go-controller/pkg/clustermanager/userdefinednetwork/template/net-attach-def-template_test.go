...
var _ = Describe("NetAttachDefTemplate", func() {
	const (
		udnTypeName = "UserDefinedNetwork"
		nadTypeName = "NetworkAttachmentDefinition"
	)
	var (
		udnApiVersion = udnv1.SchemeGroupVersion.String()
		nadApiVersion = netv1.SchemeGroupVersion.String()
	)
	DescribeTable("should fail given",
		func(spec *udnv1.UserDefinedNetworkSpec) {
			udn := &udnv1.UserDefinedNetwork{
				TypeMeta:   metav1.TypeMeta{APIVersion: udnApiVersion, Kind: udnTypeName},
	...
	It("should create net attach from spec", func() {
		...
		expectedNAD := &netv1.NetworkAttachmentDefinition{
			TypeMeta: metav1.TypeMeta{Kind: nadTypeName, APIVersion: nadApiVersion},
