...
##
## Creates development environment for testing Kubevirt cross-cluster
## live migration (CCLM) with OVN-Kubernetes Localnet user-defined networks.
##
## The environment consist of two KinD clusters, source and target clusters,
## where the source cluster is where the VM initially created and then migrated
## to the target cluster.
## Each cluster has OVN-Kubernetes, Kubevirt installed.
##
## The clusters are connected with each other over the following networks:
## - kind: Clusters nodes network
## - migration: Dedicated network migration for Kubevirt CCLM
## - underlay: Secondary network for tested VMs
##
## OPTIONS:
## --cluster-up             Spin up two KinD clusters with OVN-Kubernetes and Kubevirt
## --cluster-down           Teardown the environment
## --cluster-sync           Sync ovn-kubernetes instances
## --cluster-sync-target    Sync source cluster ovn-kubernetes instance on
## --cluster-sync-source    Sync target cluster ovn-kubernetes instance on
## --register-kubevirt      Register source and target cluster kubevirt instances to enable CCLM
...
cluster_up() {
    ...
    echo "Creating migration network NAD on taget cluster ($CLUSTER_TARGET).."
...
help() {
  ...
  echo ""
  echo "Creates development environment for testing Kubevirt cross-cluster"
  echo "live migration (CCLM) with OVN-Kubernetes Localnet user-defined networks."
  echo """"
  ...
  echo "- migration: Dedicated network migration for Kubevirt CCLM"
  echo "- underlay: Secondary network for tested VMs"
  echo ""
  echo "OPTIONS:"
